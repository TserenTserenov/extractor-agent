...
Knowledge Extractor (KE) — ИИ-система с **7 процессами**, работающими по единому пайплайну: обнаружение → классификация → маршрутизация → формализация → валидация.

**Ключевое решение:** KE **не следит** за сессией в реальном времени. Он вызывается в конкретные моменты:

| # | Триггер | Процесс | Вызывающий |
|---|---------|---------|-----------|
| 1 | Мелкое правило (1-3 строки) | — | Claude напрямую (**не KE**) |
| 2 | Исчезающая заметка (TG) | Inbox-Check | launchd / ручной |
| 3 | Inbox по расписанию | Inbox-Check | launchd (2-3ч) |
| 4 | Закрытие сессии | Session-Close Extraction | Claude (root/repo) → протокол Close |
| 5 | Явная команда | On-Demand Extraction | Claude (root/repo) |
| 6 | Загрузка документа | Bulk Extraction | Claude (root/repo) |
| 7 | Изменение в Pack | Cross-Repo Sync | Claude / git hook |
| 8 | Изменение ontology.md | Ontology Sync | Claude / git hook |
| 9 | Периодический аудит | Knowledge Audit | launchd (ежемесячно) / ручной |

> **Полная карта триггеров и архитектура вызовов** — source-of-truth: `DP.AISYS.013` § 4.0

### Архитектура вызовов

| Вызывающий | Как запускает KE | Какие процессы |
|-----------|------------------|----------------|
| **Claude root** (`~/Github`) | Читает промпт из `DS-extractor-agent/prompts/` | Session-Close, On-Demand, Bulk |
| **Claude repo** (`<repo>/`) | Читает промпт из `DS-extractor-agent/prompts/` | Session-Close, On-Demand, Bulk |
| **launchd** | Запуск Claude Code с промптом | Inbox-Check, Knowledge Audit |
| **git hook / событие** | Запуск Claude Code с промптом | Cross-Repo Sync, Ontology Sync |

### Исключение: мелкие правила (сценарий #1)

> **Capture-to-pack для мелких правил** (1-3 строки, одобрены пользователем) — Claude записывает напрямую в `CLAUDE.md` или `memory/`, **без вызова KE**. Это единственный сценарий, обходящий Экстрактор.
>
> Все остальные типы знания (доменное, различения, методы, failure modes, work products) → **только через KE**.
...
- [ ] **Онтология (SPF.SPEC.002):** согласован ли с `ontology.md` Pack'а? (типы, термины, связи)
- [ ] **Развитие онтологии:** вводит ли кандидат новый тип/термин? Если да → добавить кандидат на обновление онтологии
...
### KE + Note-Review (Стратег)

Note-Review (23:00) классифицирует заметки из fleeting-notes.md. Категория «Знание» → запись в `DS-my-strategy/inbox/captures.md`. Экстрактор проверяет inbox периодически и обрабатывает по стандартному пайплайну. Запись в Pack — только после одобрения пользователя.

```
Note-Review (23:00):
  Заметка «паттерн X» → классификация: Знание → DS-my-strategy/inbox/captures.md
  ↓
Экстрактор (Inbox-Check):
  captures.md → пайплайн → Extraction Report → одобрение → Pack
```

### KE + Capture-to-Pack (CLAUDE.md § 2, Work)

Capture-to-Pack во время Work-протокола остаётся **примитивом**: Claude Code анонсирует «Capture: X → Y» при обнаружении знания на рубеже.

**Новый порядок (с inbox):**
1. Во время Work → capture-to-pack анонсирует «Capture: X → Y»
2. На Close → одобренные captures записываются в `DS-my-strategy/inbox/captures.md`
3. Экстрактор (Inbox-Check) → формализует → предлагает → пользователь одобряет → Pack

> **Альтернатива:** KE может формализовать прямо в сессии (как раньше), но для асинхронной работы — captures идут через inbox.

---

## Процесс 6: Inbox-Check (проверка inbox)

> Периодическая обработка captures из inbox.
> Промпт: `prompts/inbox-check.md`

### Триггер

Расписание (launchd, каждые 2-3 часа, рабочие часы) или ручной запуск.
...
| Вход | Источник |
|------|----------|
| Pending captures | `DS-my-strategy/inbox/captures.md` |
| Файлы в `DS-my-strategy/inbox/` | Документы для bulk extraction (от пользователя) |
| Pack-манифесты | `00-pack-manifest.md` |

**Действия:**

```
1. Прочитать DS-my-strategy/inbox/captures.md → есть ли pending записи?
2. Прочитать DS-my-strategy/inbox/ → есть ли файлы для bulk extraction?
3. Если inbox пуст → завершить
4. Для каждого capture: стандартный пайплайн
   (классификация → маршрутизация → формализация → валидация)
5. Для каждого файла: Bulk Extraction (Процесс 3)
6. Сформировать Extraction Report
7. Отправить уведомление пользователю (Telegram)
8. Дождаться одобрения пользователя
9. Применить одобренные → записать в Pack
10. Очистить обработанные записи из DS-my-strategy/inbox/captures.md
```
...
| Выход | Формат |
|-------|--------|
| Extraction Report | Уведомление в Telegram + файл |
| Pack-совместимые файлы | `.md` с frontmatter (после одобрения) |
| Очищенный inbox | Обработанные записи удалены |

> **Принцип:** Экстрактор предлагает, пользователь решает. Автоматическая запись в Pack запрещена.

---

## Процесс 7: Ontology Sync

> Синхронизация мастер-онтологии (`DS-my-strategy/ontology.md`) с Pack-онтологиями и Downstream.

### Триггер

- Изменение `ontology.md` в любом Pack (после коммита)
- Создание нового Pack (новый контекст)
- Запрос пользователя: «синхронизируй онтологию»

### ВДВ (Вход → Действие → Выход)

| Вход | Действие | Выход |
|------|----------|-------|
| Pack ontology.md (изменённый) | Сравнить с мастер-онтологией | Ontology Sync Report |
| Мастер-онтология (DS-my-strategy/ontology.md) | Найти расхождения | Предложение обновлений |
| Downstream ontology.md (все) | Проверить актуальность ссылок | Обновлённые файлы (после одобрения) |

### Алгоритм

1. **Определить изменения** — что изменилось в Pack ontology.md (git diff)
2. **Сравнить с мастером** — какие понятия отсутствуют, изменены, удалены
3. **Проверить Downstream** — ссылаются ли Downstream на актуальные понятия
4. **Сформировать Ontology Sync Report** — таблица расхождений + предложения
5. **Применить одобренные** — обновить мастер + Downstream, закоммитить

### Правило двунаправленности

- Pack → мастер: новое понятие в Pack обязано появиться в мастер-онтологии
- Мастер → Pack: каждое понятие в мастере обязано иметь Pack-источник (нет «висящих»)
- Downstream → Pack: Downstream только ссылается, не создаёт доменных понятий

### Промпт

`DS-extractor-agent/prompts/ontology-sync.md`

---

## Моменты вызова (полная карта)

> Source-of-truth: `DP.AISYS.013` § 4.0. Таблица ниже — зеркало для быстрого доступа.

| # | Триггер | Процесс | Вызывающий |
|---|---------|---------|-----------|
| 1 | Мелкое правило (1-3 строки) | — (capture-to-pack напрямую) | Claude |
| 2 | Исчезающая заметка (TG) | Inbox-Check | launchd |
| 3 | Inbox по расписанию | Inbox-Check | launchd (2-3ч) |
| 4 | Закрытие сессии | Session-Close Extraction | Claude → протокол Close |
| 5 | Явная команда | On-Demand Extraction | Claude (`/extract`) |
| 6 | Загрузка документа | Bulk Extraction | Claude |
| 7 | Изменение в Pack | Cross-Repo Sync | Claude / git hook |
| 8 | Изменение ontology.md | Ontology Sync | Claude / git hook |
| 9 | Периодический аудит | Knowledge Audit | launchd / ручной |
